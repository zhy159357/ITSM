package com.ruoyi.activiti.controller.itsm.es;
...
public class BusinessController extends BaseController {
    ...
    public String business(ModelMap mmap) {
        ...
        String status = businessContentService.getStatusByUserId(ogUser.getUserId());
        mmap.addAttribute("status", status);
        mmap.addAttribute("user", ogUser.getLoginName());
        return prefix + "/cont1";
    }
    ...
    public AjaxResult addSave(@Validated KnowledgeContent knowledgeContent, KnowledgeSearch knowledgeSearch) {
        ...
        if (StringUtils.isEmpty(cont_id)) {
            ...
            int row = businessContentService.insertContent(knowledgeContent);

            String userId = ShiroUtils.getOgUser().getUserId();
            String loginName = ShiroUtils.getOgUser().getLoginName();
            ...
            if(row >0){
                ...
                if ("3".equals(cstatus)) {
                    //短信内容
                    smsText ="二线厂商的:"+ loginName+"，提交了业务知识,知识标题："+knowledgeContent.getTitle()+"，请您及时审批！";
                    //获取二线管理员审批人信息
                    String twoAuditorId = knowledgeContent.getTwoAuditorId();
                    if(!StringUtils.isEmpty(twoAuditorId)){
                        //获取Person信息
                        OgPerson ogPerson = personService.selectOgPersonById(twoAuditorId);
                        if(ogPerson != null){
                            try{
                                sendSms(ogPerson,smsText);
                                logger.debug("--------2新提交业务知识短信--------"+ogPerson.getpName()+"------"+smsText);
                            }catch (Exception e){
                                e.printStackTrace();
                                logger.error("二线厂商新提交业务知识短信失败:{} ",e);
                            }
                ...
                if("4".equals(cstatus)){
                    //短信内容
                    smsText = "二线管理员的:"+ loginName+"，提交了业务知识,知识标题："+knowledgeContent.getTitle()+"，请您及时审批！";
                    //获取一线管理员审批人信息
                    String oneAuditorId = knowledgeContent.getOneAuditorId();
                    if(!StringUtils.isEmpty(oneAuditorId)){
                        //获取Person信息
                        OgPerson ogPerson = personService.selectOgPersonById(oneAuditorId);
                        if(ogPerson != null){
                            try{
                                sendSms(ogPerson,smsText);
                                logger.debug("--------1新提交业务知识短信--------"+ogPerson.getpName()+"------"+smsText);
                            }catch (Exception e){
                                e.printStackTrace();
                                logger.error("二线管理员新提交业务知识短信失败:{} ",e);
                            }
                        }
                    }
                }
...
//        knowledgeContent.setDescribes("");
        ...
        if(StringUtils.isEmpty(knowledgeContent.getId())){
            ...
            row = businessContentService.insertContent(knowledgeContent);
        } else {
            row = businessContentService.updateCont(knowledgeContent);
        }
    ...
    public AjaxResult editSave(@Validated KnowledgeContent knowledgeContent) {
        ...
        int row = businessContentService.updateCont(knowledgeContent);
                ...
                if(!StringUtils.isEmpty(twoAuditorId)){
                    ...
                    if(ogPerson != null){
                        try{
                            ...
                            e.printStackTrace();
                            logger.error("二线厂商的修改提交业务知识短信失败:{} ",e);
                        }
                    }
                }
            }
                ...
                if(!StringUtils.isEmpty(oneAuditorId)){
                    ...
                    if(ogPerson != null){
                        try{
                            ...
                            e.printStackTrace();
                            logger.error("二线管理员的修改提交业务知识短信失败:{} ",e);
                        }
                    }
                }
            }
...
//        }
    ...
    {
        int row = businessContentService.auditBusiness(knowledgeContent);
        String userId = ShiroUtils.getOgUser().getUserId();
        String loginName = ShiroUtils.getOgUser().getLoginName();
        String cstatus = businessContentService.getStatusByUserId(userId);
        String smsText = "";
        ...
        if(row >0){
            ...
            if("4".equals(cstatus)){
                //短信内容
                smsText = "二线管理员的:"+ loginName+"，业务知识初审通过,知识标题："+knowledgeContent.getTitle()+"，请您及时审批！";
                //获取一线管理员审批人信息
                String oneAuditorId = knowledgeContent.getOneAuditorId();
                if(!StringUtils.isEmpty(oneAuditorId)){
                    //获取Person信息
                    ...
                    if(ogPerson != null){
                        try{
                            sendSms(ogPerson,smsText);
                            logger.debug("--------1业务知识初审通过短信--------"+ogPerson.getpName()+"------"+smsText);
                        }catch (Exception e){
                            e.printStackTrace();
                            logger.error("二线管理员的的业务知识初审通过短信失败:{} ",e);
                        }
    ...
    public AjaxResult noPass(@Validated KnowledgeContent knowledgeContent)
    {
        ...
        knowledgeContent.setStatus("0");
        int row = businessContentService.auditContent(knowledgeContent);
        ...
        if(row >0){
            //二线管理员
            if("4".equals(cstatus)){
                if(StringUtils.isEmpty(knowledgeContent.getReason())){
                    //短信内容
                    smsText = "二线管理员的:"+ loginName+"，回退了您的业务知识,知识标题："+knowledgeContent.getTitle()+"，请注意查看！";
                }else {
                    //短信内容
                    smsText = "二线管理员的:"+ loginName+"，回退了您的业务知识,知识标题："+knowledgeContent.getTitle()+ "。退回理由："+ knowledgeContent.getReason()  +"，请注意查看！";
                }
                //获取知识创建人
                String createId = businessContentService.selectBusinessCreateIdById(knowledgeContent.getId());

                if(!StringUtils.isEmpty(createId)){
                    //获取Person信息
                    OgPerson ogPerson = personService.selectOgPersonById(createId);
                    if(ogPerson != null){
                        try{
                            sendSms(ogPerson,smsText);
                            logger.debug("--------业务知识回退短信--------"+ogPerson.getpName()+"------"+smsText);
                        }catch (Exception e){
                            e.printStackTrace();
                            logger.error("二线管理员的业务知识回退短信失败:{} ",e);
                        }
                    }
                }
            }
            ...
            if("2".equals(cstatus)){
                if(StringUtils.isEmpty(knowledgeContent.getReason())){
                    //短信内容
                    smsText = "一线管理员的:"+ loginName+"，终审回退了您的业务知识,知识标题："+knowledgeContent.getTitle()+"，请注意查看！";
                }else {
                    //短信内容
                    smsText = "一线管理员的:"+ loginName+"，终审回退了您的业务知识,知识标题："+knowledgeContent.getTitle()+ "。退回理由："+ knowledgeContent.getReason()  +"，请注意查看！";
                }
                //获取知识创建人
                String createId = businessContentService.selectBusinessCreateIdById(knowledgeContent.getId());
                ...
                if(!StringUtils.isEmpty(createId)){
                    ...
                    if(ogPerson != null){
                        try{
                            sendSms(ogPerson,smsText);
                            logger.debug("--------业务知识终审回退短信--------"+ogPerson.getpName()+"------"+smsText);
                        }catch (Exception e){
                            e.printStackTrace();
                            logger.error("一线管理员的业务知识终审回退短信失败:{} ",e);
                        }
...
//    	}
        ...
        List<OgUser> resultList = list2.stream()
                .filter(item -> !list1.stream().map(e -> e.getUserId()).collect(Collectors.toList()).contains(item.getUserId()))
    ...
    public AjaxResult applyBackSave(@Validated KnowledgeContent knowledgeContent)
    {
        ...
        int row = businessContentService.backApply(knowledgeContent);
        ...
        if(row >0){
            ...
            if ("3".equals(cstatus)) {
                //短信内容
                smsText ="二线厂商的:"+ loginName+"，提交了业务知识申请修改下线,知识标题："+knowledgeContent.getTitle()+"，请您及时审批！";
                //获取二线管理员审批人信息
                String twoAuditorId = knowledgeContent.getTwoAuditorId();
                if(!StringUtils.isEmpty(twoAuditorId)){
                    //获取Person信息
                    ...
                    if(ogPerson != null){
                        try{
                            sendSms(ogPerson,smsText);
                            logger.debug("--------2新提交业务知识申请修改下线短信--------"+ogPerson.getpName()+"------"+smsText);
                        }catch (Exception e){
                            e.printStackTrace();
                            logger.error("二线厂商的提交业务知识申请修改下线短信失败:{} ",e);
                        }
            ...
            if("4".equals(cstatus)){
                //短信内容
                smsText = "二线管理员的:"+ loginName+"，提交了业务知识申请修改下线,知识标题："+knowledgeContent.getTitle()+"，请您及时审批！";
                //获取一线管理员审批人信息
                String oneAuditorId = knowledgeContent.getOneAuditorId();
                if(!StringUtils.isEmpty(oneAuditorId)){
                    //获取Person信息
                    ...
                    if(ogPerson != null){
                        try {
                            sendSms(ogPerson,smsText);
                            logger.debug("--------1新提交业务知识申请修改下线短信--------"+ogPerson.getpName()+"------"+smsText);
                        }catch (Exception e){
                            e.printStackTrace();
                            logger.error("二线管理员的提交业务知识申请修改下线短信失败:{} ",e);
                        }
                    }
                }
            }
    ...
    public AjaxResult applyTrashSave(@Validated KnowledgeContent knowledgeContent)
    {
        ...
        int row = businessContentService.trashApply(knowledgeContent);
        ...
        if(row >0){
            ...
            if ("3".equals(cstatus)) {
                //短信内容
                smsText ="二线厂商的:"+ loginName+"，提交了业务知识申请做废,知识标题："+knowledgeContent.getTitle()+"，请您及时审批！";
                //获取二线管理员审批人信息
                String twoAuditorId = knowledgeContent.getTwoAuditorId();
                if(!StringUtils.isEmpty(twoAuditorId)){
                    //获取Person信息
                    ...
                    if(ogPerson != null){
                        try{
                            sendSms(ogPerson,smsText);
                            logger.debug("--------2新提交业务知识申请做废短信--------"+ogPerson.getpName()+"------"+smsText);
                        }catch (Exception e){
                            e.printStackTrace();
                            logger.error("二线厂商的提交业务知识申请做废短信失败:{} ",e);
                        }
                    }
                }
            }
            ...
            if("4".equals(cstatus)){
                //短信内容
                smsText = "二线管理员的:"+ loginName+"，提交了业务知识申请做废,知识标题："+knowledgeContent.getTitle()+"，请您及时审批！";
                //获取一线管理员审批人信息
                String oneAuditorId = knowledgeContent.getOneAuditorId();
                if(!StringUtils.isEmpty(oneAuditorId)){
                    //获取Person信息
                    ...
                    if(ogPerson != null){
                        try{
                            sendSms(ogPerson,smsText);
                            logger.debug("--------1新提交业务知识申请做废短信--------"+ogPerson.getpName()+"------"+smsText);
                        }catch (Exception e){
                            e.printStackTrace();
                            logger.error("二线管理员的提交业务知识申请做废短信失败:{} ",e);
                        }
                    }
                }
            }
    ...
    public AjaxResult forceBackSave(@Validated KnowledgeContent knowledgeContent)
    {
        ...
        int row = businessContentService.forceBack(knowledgeContent);
        return toAjax(row);
    }
    ...
    public AjaxResult forceTrashSave(@Validated KnowledgeContent knowledgeContent)
    {
        ...
        int row = businessContentService.forceTrash(knowledgeContent);
        return toAjax(row);
    }
