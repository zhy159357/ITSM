 ...
 * 变更请求单
 */
@Service("benchAdapterImplCmBizQingQiu")
public class BenchAdapterImplCmBizQingQiu extends AbstractBenchAdapter {
        ...
        if (!reList.isEmpty()) {
            ...
            for (Map<String, Object> map : reList) {
                ...
                if (StringUtils.isNotEmpty(business_key)) {
                    CmBizQingqiu cmBizQingqiu = cmBizQingqiuService.selectCmBizQingqiuById(business_key);//变更请求单VO
                    if (cmBizQingqiu != null) {
                        benchTask = new BenchTask();
                        benchTask.setType("BGQQ");//任务类型
                        benchTask.setTaskId(map.get("taskId").toString());//任务ID
                        benchTask.setBizId(business_key);//业务ID
                        benchTask.setNumber(cmBizQingqiu.getChangeCode());//单号--
                        benchTask.setTitle(cmBizQingqiu.getChangeSingleName());//标题--
