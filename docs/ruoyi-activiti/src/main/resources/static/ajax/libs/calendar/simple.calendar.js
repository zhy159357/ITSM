...
function lbGetNowData(t){
	if($(t).attr('class').indexOf('sc-othermenth')==-1){
	   var o=$(t).parents('.sc-body').siblings('.sc-header');
	   var a=o.find('.sc-select-year').val();
	   var b=o.find('.sc-select-month').val();if(b<10){b='0'+b}
	   var c=$(t).find('.day').text();if(c<10){c='0'+c}
	   var d=a+'-'+b+'-'+c;
	   return d;
	}
}
function lbAdd1411(a,b,c){
	 var d="{";var d1="}";var tt="";
	 ...
	 a._defaultOptions.mark=JSON.parse(d);
	 a.update();
 }
 function lbAdd1437(a,b,c){
	 $("#"+a).find('.sc-select-year').val(b);
	 $("#"+a).find('.sc-select-year').change();

	 $("#"+a).find('.sc-select-month').val(c);
	 $("#"+a).find('.sc-select-month').change();
 }
...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function create() {
      ...
      //actions
      header.innerHTML = header.innerHTML + '<div class="sc-actions" style="display:none;">' + '      <div class="sc-yleft">' + '        &lsaquo;</div>' + '      <select class="sc-select-year" name="">' + '      </select>' + '      <div class="sc-yright">&rsaquo;</div>' + '  </div>';
      header.innerHTML = header.innerHTML + '<div class="sc-actions"style="display:none;">' + '    <div class="sc-mleft">' + '      &lsaquo;</div>' + '    <select class="sc-select-month" name="">' + '    </select>' + '    <div class="sc-mright">&rsaquo;</div>' + '</div>';
    ...
    value: function getSelectedDay() {
	  if($(this.selectDay.querySelector('.day')).parent('.sc-othermenth').length==1){
		  if($(this.selectDay.querySelector('.day')).html()<15){
			  var selectMonth = parseInt(this.container.querySelector('.sc-select-month').value)+1;
//			  console.log(selectMonth);
		  }else if($(this.selectDay.querySelector('.day')).html()>15){
			  var selectMonth = parseInt(this.container.querySelector('.sc-select-month').value)-1;
//			  console.log(selectMonth);
		  }
	  }else{
	  ...
	  var d=new Date(selectYear, selectMonth - 1, selectDay);
	  var datetime=d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate();
	  var a=datetime.split('-');console.log(a);
      if(a[1]<10){a[1]='0'+a[1]}
      if(a[2]<10){a[2]='0'+a[2]}
      datetime=a[0]+'-'+a[1]+'-'+a[2];
      return datetime
    }
