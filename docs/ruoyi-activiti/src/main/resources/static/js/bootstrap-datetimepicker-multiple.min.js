var lbDataArr20210125=[];var lbaarr=[];var lbdatadiv=null;
(function(a) {
	...
	function d() {
		var j = new Date();
		return h(j.getUTCFullYear(), j.getUTCMonth(), j.getUTCDate(), j.getUTCHours(), j.getUTCMinutes(), j.getUTCSeconds(),
			0)
	}
	var i = function(l, k) {
		var n = this;
		...
		this.multiple = k.multiple || false;
		if (this.multiple) {
			k.autoclose = false;
			k.minView = 3
		}
		...
		this.format = g.parseFormat(k.format || this.element.data("date-format") || a[this.language].format || g.getDefaultFormat(
			this.formatType, "input"), this.formatType);
		this.isInline = false;
		this.isVisible = false;
		this.isInput = this.element.is("input");
		this.fontAwesome = k.fontAwesome || this.element.data("font-awesome") || false;
		this.bootcssVer = k.bootcssVer || (this.isInput ? (this.element.is(".form-control") ? 3 : 2) : (this.bootcssVer =
			this.element.is(".input-group") ? 3 : 2));
		this.component = this.element.is(".date") ? (this.bootcssVer == 3 ? this.element.find(
		...
		this.linkFormat = g.parseFormat(k.linkFormat || this.element.data("link-format") || g.getDefaultFormat(this.formatType,
			"link"), this.formatType);
	...
	i.prototype = {
		...
		setValue: function() {
			if (this.multiple) {
				var j = [];
				for (var k in this.viewDates) {
					j.push(g.formatDate(new Date(this.viewDates[k]), this.format, this.language, this.formatType))
				}
				var l = j.join(",")
			} else {
				var l = this.getFormattedDate()
			}
			if (!this.isInput) {
				if (this.component) {
					this.element.find("input").val(l)
				}
				this.element.data("date", l)
			} else {
				this.element.val(l)
			}
			if (this.linkField) {
		...
		setDatesDisabled: function(k) {
			this.datesDisabled = k || [];
			if (!f.isArray(this.datesDisabled)) {
				this.datesDisabled = this.datesDisabled.split(/,\s*/)
			}
			var j = this;
			this.datesDisabled = f.map(this.datesDisabled, function(l) {
				return g.parseDate(l, j.format, j.language, j.formatType).toDateString()
			});
			this.update();
			this.updateNavArrows()
		},
		...
		update: function() {
			...
			if (arguments && arguments.length && (typeof arguments[0] === "string" || arguments[0] instanceof Date)) {
				...
				k = (this.isInput ? this.element.val() : this.element.find("input").val()) || this.element.data("date") || this
					.initialDate;
				if (typeof k == "string" || k instanceof String) {
					k = k.replace(/^\s+|\s+$/g, "")
				}
			}
			if (this.multiple && typeof k == "string") {
				var o = k.split(",");
				k = false;
				for (var m in o) {
					var l = g.parseDate(o[m], this.format, this.language, this.formatType);
					var j = (new h(l.getUTCFullYear(), l.getUTCMonth(), l.getUTCDate())).valueOf();
					if (f.inArray(j, this.viewDates) === -1) {
						this.viewDates.push(j)
					}
				}
			}
			if (!k) {
			...
			this.date = g.parseDate(k, this.format, this.language, this.formatType);
			if (n) {
				this.setValue()
			}
		...
		fill: function() {
			...
			while (K.valueOf() < j) {
				...
				if (this.todayHighlight && K.getUTCFullYear() == G.getFullYear() && K.getUTCMonth() == G.getMonth() && K.getUTCDate() ==
					G.getDate()) {
					v += " today"
				}
				if (this.multiple) {
					if (f.inArray(K.valueOf(), this.viewDates) !== -1) {
						v += " active"
					}
				} else {
					if (K.valueOf() == r) {
				...
				if ((K.valueOf() + 86400000) <= this.startDate || K.valueOf() > this.endDate || f.inArray(K.getUTCDay(), this.daysOfWeekDisabled) !==
					-1 || f.inArray(K.toDateString(), this.datesDisabled) !== -1) {
					v += " disabled"
				}
			...
			if (L == u) {
				var o = p.length - 12;
				p.eq(this.date.getUTCMonth() + o).addClass("asd")
			}
			if (u < z || u > q) {
				p.addClass("disabled")
			}
			...
			u -= 1;
			for (var B = -1; B < 11; B++) {
				s += '<span class="year' + (B == -1 || B == 10 ? " old" : "") + (L == u ? " active" : "") + (u < z || u > q ?
					" disabled" : "") + '">' + u + "</span>";
				u += 1
			}
			J.html(s);
			...
			switch (this.viewMode) {
				case 0:
					if (this.startDate !== -Infinity && l <= this.startDate.getUTCFullYear() && m <= this.startDate.getUTCMonth() &&
						k <= this.startDate.getUTCDate() && j <= this.startDate.getUTCHours()) {
					...
					if (this.endDate !== Infinity && l >= this.endDate.getUTCFullYear() && m >= this.endDate.getUTCMonth() && k >=
						this.endDate.getUTCDate() && j >= this.endDate.getUTCHours()) {
		...
		click: function(n) {
			n.stopPropagation();
			n.preventDefault();
			if($(n.target).is('.month')){
				lbdatadiv=$(n.target).parents('.datetimepicker-dropdown-bottom-right.dropdown-menu').attr('class');
				var lbindex20210125=$(n.target).index();
				var lbyear20210125=$(n.target).parents('tbody').siblings('thead').find('.switch').text();
				lbDataArr20210125.push(lbyear20210125);
				for(var lbt1=0;lbt1<lbDataArr20210125.length;lbt1++){
					if(lbDataArr20210125[lbt1]==lbyear20210125){
						lbDataArr20210125[lbt1]+=(','+lbindex20210125);
					}
				}
				console.log(lbDataArr20210125);
			}
			...
			if (o.length == 1) {
				...
				//lbDataArr20210125
				lbdatadiv=lbdatadiv.split(' ')[0];
				console.log(lbdatadiv);
				var lb1260text=$('.'+lbdatadiv).find('.datetimepicker-months').find('tr').eq(0).find('.switch').text();
				var lb1260td=$('.'+lbdatadiv).find('.datetimepicker-months').find('td');
				for(var lbms=0;lbms<lbDataArr20210125.length;lbms++){
					var lb1237=lbDataArr20210125[lbms].split(',');
					if(lb1237[0]==lb1260text){
						console.log(lb1260td.children('.month').eq(lb1237[1]).attr('class').indexOf('active'));
						if(lb1260td.children('.month').eq(lb1237[1]).attr('class').indexOf('active')==-1){
							lb1260td.children('.month').eq(lb1237[1]).addClass('active');
						}else{
							lb1260td.children('.month').eq(lb1237[1]).removeClass('active');
						}
		...
		_setDate: function(k, m) {
			...
			if (this.multiple) {
				var j = (new h(this.date.getUTCFullYear(), this.date.getUTCMonth(), this.date.getUTCDate())).valueOf();
				if (f.inArray(j, this.viewDates) === -1) {
					this.viewDates.push(j)
				} else {
					this.viewDates.splice(f.inArray(j, this.viewDates), 1)
				}
			}
			this.fill();
			this.setValue();
		...
		showMode: function(j) {
			...
			this.picker.find(">div").hide().filter(".datetimepicker-" + g.modes[this.viewMode].clsName).css("display",
				"block");
			this.updateNavArrows()
		},
		reset: function(j) {
	...
	var g = {
		...
		parseDate: function(n, w, q, u) {
			...
			if (/^[-+]\d+[dmwy]([\s,]+[-+]\d+[dmwy])*$/.test(n)) {
				...
				return h(n.getUTCFullYear(), n.getUTCMonth(), n.getUTCDate(), n.getUTCHours(), n.getUTCMinutes(), n.getUTCSeconds(),
					0)
			}
			var o = n && n.toString().match(this.nonpunctuation) || [],
				n = new Date(0, 0, 0, 0, 0, 0, 0),
				t = {},
				v = ["hh", "h", "ii", "i", "ss", "s", "yyyy", "yy", "M", "MM", "m", "mm", "D", "DD", "d", "dd", "H", "HH", "p",
					"P"
				],
				x = {
					hh: function(B, s) {
		...
		headTemplate: "<thead>" + "<tr>" + '<th class="prev"><i class="{iconType} {leftArrow}"/></th>' +
			'<th colspan="5" class="switch"></th>' + '<th class="next"><i class="{iconType} {rightArrow}"/></th>' + "</tr>" +
			"</thead>",
		headTemplateV3: "<thead>" + "<tr>" + '<th class="prev"><span class="{iconType} {leftArrow}"></span> </th>' +
			'<th colspan="5" class="switch"></th>' + '<th class="next"><span class="{iconType} {rightArrow}"></span> </th>' +
			"</tr>" + "</thead>",
		contTemplate: '<tbody><tr><td colspan="7"></td></tr></tbody>',
		footTemplate: "<tfoot>" + '<tr><th colspan="7" class="today"></th></tr>' +
			'<tr><th colspan="7" class="clear"></th></tr>' + "</tfoot>"
	};
	g.template = '<div class="datetimepicker">' + '<div class="datetimepicker-minutes">' +
		'<table class=" table-condensed">' + g.headTemplate + g.contTemplate + g.footTemplate + "</table>" + "</div>" +
		'<div class="datetimepicker-hours">' + '<table class=" table-condensed">' + g.headTemplate + g.contTemplate + g.footTemplate +
		"</table>" + "</div>" + '<div class="datetimepicker-days">' + '<table class=" table-condensed">' + g.headTemplate +
		"<tbody></tbody>" + g.footTemplate + "</table>" + "</div>" + '<div class="datetimepicker-months">' +
		'<table class="table-condensed">' + g.headTemplate + g.contTemplate + g.footTemplate + "</table>" + "</div>" +
		'<div class="datetimepicker-years">' + '<table class="table-condensed">' + g.headTemplate + g.contTemplate + g.footTemplate +
		"</table>" + "</div>" + "</div>";
	g.templateV3 = '<div class="datetimepicker">' + '<div class="datetimepicker-minutes">' +
		'<table class=" table-condensed">' + g.headTemplateV3 + g.contTemplate + g.footTemplate + "</table>" + "</div>" +
		'<div class="datetimepicker-hours">' + '<table class=" table-condensed">' + g.headTemplateV3 + g.contTemplate + g.footTemplate +
		"</table>" + "</div>" + '<div class="datetimepicker-days">' + '<table class=" table-condensed">' + g.headTemplateV3 +
		"<tbody></tbody>" + g.footTemplate + "</table>" + "</div>" + '<div class="datetimepicker-months">' +
		'<table class="table-condensed">' + g.headTemplateV3 + g.contTemplate + g.footTemplate + "</table>" + "</div>" +
		'<div class="datetimepicker-years">' + '<table class="table-condensed">' + g.headTemplateV3 + g.contTemplate + g.footTemplate +
		"</table>" + "</div>" + "</div>";
