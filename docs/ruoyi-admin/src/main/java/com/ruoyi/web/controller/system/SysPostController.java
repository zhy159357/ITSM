...
public class SysPostController extends BaseController
{
    ...
    @PostMapping("/list")
    @ResponseBody
    public TableDataInfo list(OgPost post)
    {
        startPage();
     ...
     * @return
     */
    @PostMapping("/postSelectList")
    @ResponseBody
    public TableDataInfo postSelectList(OgPost post)
    {
        //获取当前账号已经分配的岗位信息
        String userId = post.getUserId(); //当前要添加岗位的userId
        OgUserPost ogUserPost = new OgUserPost();
        ogUserPost.setUserId(userId);
        List<OgUserPost> ogUserPosts = ogUserPostService.selectPostByUserId(ogUserPost);
        // 是否全国中心，是-true
        Boolean centerFlag = "1".equals(deptService.getIsCenter());
        if(StringUtils.isNotEmpty(ogUserPosts)){
            List<String> postIds = new ArrayList<>();
            ogUserPosts.stream().forEach(userPost ->postIds.add(userPost.getPostId()));
            // getIsCenter方法返回1代表是全国中心，0表示省中心
            startPage();
            List<OgPost> ogPosts = ogPostService.selectBatchPostList(postIds,post.getPostName(),centerFlag);
            return getDataTable(ogPosts);
        }else{
            post.getParams().put("centerFlag", centerFlag);
            startPage();
            List<OgPost> list = ogPostService.selectPostList(post);
            return getDataTable(list);
        }
    ...
    {
        /*String postCode = String.valueOf(new Random().nextInt(99999999));
        mmap.put("postId",postCode);*/
        return prefix + "/add";
    }
