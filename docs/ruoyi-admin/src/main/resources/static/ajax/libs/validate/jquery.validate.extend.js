...
$(document).ready(function(){
	...
	},"请填写正确的座机号码,格式如下：0xx-xxxxxxx或是0xxx-xxxxxxx");
	//姓名校验
	...
	jQuery.validator.addMethod("isEightNum",function(value,element){
		var nums = /^(\d{8})$/;
		return this.optional(element) || (nums).test(value);
	},"格式不正确，必须为8位数字");
	//工号必须八位数字
	...
	//人员顺序必须为正整数
	jQuery.validator.addMethod("isSortNum",function(value,element){
		var sortnums = /^[1-9]\d*$/;
		return this.optional(element) || (sortnums).test(value);
	},"顺序必须为正整数");
	//密码校验规则表达式，数字、字母，大小写、特殊字符，8位长度
	jQuery.validator.addMethod("isPwd",function(value,element){
		var nums = /^(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[~!@#$%^&*_.]).{8,}$/;
		return this.optional(element) || (nums).test(value);
	},"数字、字母，大小写、特殊字符，8位长度");
	//校验新旧密码是否相同
