...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 导出当前页数据
            exportExcelCurrentPage: function(formId) {
                var length = $.table.tableCount();
                if(length == 0){
                    $.modal.alertWarning("列表数据为空，不做导出数据处理！");
                    return;
                ...
                table.set();
                $.modal.confirm("确定导出" + table.options.modalName + "当前页的数据吗？", function() {
                    var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                    var params = $("#" + table.options.id).bootstrapTable('getOptions');
                    var dataParam = $("#" + currentId).serializeArray();
                    dataParam.push({ "name": "orderByColumn", "value": params.sortName });
                    dataParam.push({ "name": "isAsc", "value": params.sortOrder });
                    dataParam.push({ "name": "params[currentPage]", "value": "currentPage" });
                    dataParam.push({ "name": "pageSize", "value": params.pageSize });
                    dataParam.push({ "name": "pageNum", "value": params.pageNumber });
                    $.modal.loading("正在导出数据，请稍后...");
                    $.post(table.options.exportUrl, dataParam, function(result) {
                        if (result.code == web_status.SUCCESS) {
                            window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
            ...
            //查询表格中记录数量
            tableCount: function() {
                return $("#" + table.options.id).bootstrapTable('getData').length;
            },
            // 查询表格首列值
            ...
            // 回显项目参数管理字典
            selectPubParaValueData : function (datas, value) {
                var actions = [];
        ...
        form: {
            ...
            // 表单重置所有，未赋值formId，默认取值'formId'
            resetAll: function(formId, tableId) {
                if(formId == undefined){
                    formId = 'formId';
                }
                $("#"+formId).find('input[type=text],select,input[type=hidden]').each(function() {
                    var calss=this.className;
                    //select2
                    if(this.dataset.select2Id!=undefined){
                        $(this).val(null).trigger("change");
                    }else
                    //bootstrap-select
                    if(calss.indexOf("selectpicker")>0){
            ...
            // 表单重置所有，未赋值formId，默认取值'formId'
            resetToAll: function(formId, tableId,tableId2) {
                if(formId == undefined){
                    formId = 'formId';
                }
                $("#"+formId).find('input[type=text],select,input[type=hidden]').each(function() {
                ...
                if (table.options.type == table_type.bootstrapTable) {
                    ...
                    if($.common.isEmpty(tableId)){
                        ...
                        $("#" + tableId).bootstrapTreeTable('refresh', []);
                        $("#" + tableId2).bootstrapTreeTable('refresh', []);
                    }
                }
            },
        ...
        modal: {
            ...
            openNoBtnFileFlush: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
                ...
                if ($.common.isEmpty(callback)) {
                    callback = function(index, layero) {
                        var iframeWin = layero.find('iframe')[0];
                        iframeWin.contentWindow.flushFileTable(index, layero);
                    }
                }
                layer.open({
                    type: 2,
            ...
            openConfirmBtn: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            openAddUser: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            openEditUser: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层遮罩   电话条用的弹框形式
            openMask: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            // 弹出层指定宽度 无按钮
            openNoColse: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if ($.common.isMobile()) {
                    width = 'auto';
                    height = 'auto';
            ...
            openOptions: function (options) {
                ...
                var flag
                if($.common.isEmpty(options.btn)){
                    flag = _btn;
                }else if('-1' == options.btn){
                    flag = '';
                }else{
                    flag = options.btn
                }
        ...
        operate: {
            ...
            // 提交数据
            cascadeSubmit: function(url, type, dataType, data, callback) {
                var config = {
                    url: url,
                    type: type,
                    dataType: dataType,
                    ...
                    success: function(result) {
                        ...
                        $.operate.cascadeAjaxSuccess(result);
                    }
                };
                $.ajax(config)
            },
            ...
            saveIds: function() {
                ...
                $.modal.confirm("确认要保存选中的" + rows.length + "条数据吗?", function() {
            ...
            removeAllCurrentUser: function() {
                ...
                $.get(prefix+'/selectOgUserByUserId',function (result) {
                    if(result.data.pId==row.person.pId){
                        //判断当前
                        $.modal.confirm("确认要删除选中的附件数据吗?", function() {
                            var url = table.options.removeUrl;
                            var data = { "ids": rows.join() };
                            $.operate.submit(url, "post", "json", data);
                        });
            ...
            removeFmSwMb: function() {
                ...
                $.get(prefix+'/selectOgUserByUserId',function (result) {
                    if(result.uname == 'admin'){
                        //判断当前
                        $.modal.confirm("确认要删除选中的模板数据吗?", function() {
                            var url = table.options.removeUrl;
                            var data = { "ids": rows.join() };
                            $.operate.submit(url, "post", "json", data);
                        });
                    }else {
                        if (result.data.pId == row.createId) {
                            //判断当前
                            $.modal.confirm("确认要删除选中的模板数据吗?", function () {
                                var url = table.options.removeUrl;
                                var data = {"ids": rows.join()};
                            ...
                            $.modal.alertError('不能删除非本人创建的模板数据 ');
                        }
                    }
                });
            ...
            cascadeRemove: function() {
                ...
                $.modal.confirm("确认要删除选中的" + rows.length + "条数据吗?", function() {
                    var url = table.options.removeUrl;
                    var data = { "ids": rows.join() };
                    $.operate.cascadeSubmit(url, "post", "json", data);
                });
            },
            // 清空信息
            clean: function() {
            ...
            detailshow: function(id) {
                ...
                if($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                    ...
                    var url = table.options.detailUrl.replace("{id}", row[table.options.uniqueId]);
                    $.modal.openDetail("详情" + table.options.modalName, url);
                } else {
                    $.modal.openDetail("详情" + table.options.modalName, $.operate.detailUrl(id));
                }
            },
            edit: function(id) {
                table.set();
                ...
                $.get(prefix+'/selectOgUserByUserId',function (result) {
                    ...
                    if(result.uname == 'admin'){
                        ...
                        if(result.data.pId==row.createId){
                            ...
                            $.modal.alertError('不能修改非本人创建的模板数据 ');
                        }
                    }
            ...
            // 处理信息，以tab页展现
            commonTab: function(id,title) {
                table.set();
                $.modal.openTab(title + table.options.modalName, $.operate.editUrl(id));
            },
            // 克隆信息，以tab页展现
            cloneTab: function(id) {
                table.set();
                $.modal.openTab("克隆" + table.options.modalName, $.operate.cloneUrl(id));
            },
            // 修改信息，以tab页展现
            fmSwTab: function(id) {
                table.set();
            ...
            cloneUrl: function(id) {
                ...
                if ($.common.isNotEmpty(id)) {
                    ...
                    url = table.options.cloneUrl.replace("{id}", id);
                }
                return url;
            },
            // 保存信息 刷新表格
            ...
            // 保存选项卡信息
            saveTabLxbg: function(url, data, callback) {
                var config = {
                    url: url,
                    type: "post",
                    dataType: "json",
                    ...
                    success: function(result) {
                        ...
                        $.operate.successTabLxbgCallback(result);
                    }
                };
                $.ajax(config)
            },
            ...
            cascadeAjaxSuccess: function (result) {
                if (result.code == web_status.SUCCESS && table.options.type == table_type.bootstrapTable) {
                    $.modal.msgSuccess(result.msg);
                    $.table.refresh('auto-table');
                    $.table.refresh('param-table');
                } else if (result.code == web_status.SUCCESS && table.options.type == table_type.bootstrapTreeTable) {
                    $.modal.msgSuccess(result.msg);
                    $.treeTable.refresh();
                } else if (result.code == web_status.SUCCESS && $.common.isEmpty(table.options.type)) {
            ...
            // 选项卡成功回调执行事件（父窗体静默更新）
            successTabLxbgCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
                    var topWindow = $(window.parent.document);
                    var currentId = $('.page-tabs-content', topWindow).find('.active').attr('data-panel');
                    var $contentWindow = $('.RuoYi_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    ...
                    $contentWindow.$(".layui-layer-padding").removeAttr("style");
                    if ($contentWindow.table.options.type == table_type.bootstrapTable) {
                        $contentWindow.$.table.refresh('bootstrap-table');
                    } else if ($contentWindow.table.options.type == table_type.bootstrapTreeTable) {
                        $contentWindow.$.treeTable.refresh('bootstrap-table');
                    }
                    $.modal.closeTab();
                } else if (result.code == web_status.WARNING) {
                    $.modal.alertWarning(result.msg)
        ...
        tree: {
            ...
            // 获取当前被勾选集合名字
            getCheckedNodesName: function(column) {
                var _column = $.common.isEmpty(column) ? "name" : column;
                var nodes = $._tree.getCheckedNodes(true);
                return $.map(nodes, function (row) {
                    return row[_column];
                }).join();
