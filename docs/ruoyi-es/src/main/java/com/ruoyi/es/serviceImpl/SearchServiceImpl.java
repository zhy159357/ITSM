...
public class SearchServiceImpl implements SearchService {
    ...
    public int updateById(KnowledgeSearch knowledgeSearch) {
        ...
        if("1".equals(knowledgeSearch.getStatus())){
            ...
            if(null==esSearchBean){
                ...
                esSearchService.save(esSearchBean);
            }else {
      ...
      //根据字符串长度获取查询粒度
        ...
        while(keyLength > 0){
            ...
            nativeSearchQueryBuilder.withQuery(QueryBuilders.matchQuery(name, keywords)
                    .minimumShouldMatch(String.valueOf(keyLength)))
                    .withPageable(new PageRequest(0,10000));
            list .addAll( elasticsearchTemplate.queryForList(nativeSearchQueryBuilder.build(), EsSearchBean.class));
            
            minuend++;
            keyLength = kword - minuend;
        }
    ...
    private int getMinimumFormatList(List<EsSearchBean> list,EsSearchBean esb, String name, String keywords){  	
        ...
        while(keyLength > 0){
            ...
            if(!list.isEmpty()){
            	for(EsSearchBean es : list){
                    if(es.getId().equals(esb.getId())){
                        return minuend;
                    }
                }
            }
    ...
    private KnowledgeContent searchMonitorExport( String keywords, String oraginkeywords, List<KnowledgeContent> selectList){
      ...
      //  long s1=System.currentTimeMillis();
        ...
        nativeSearchQueryBuilder.withQuery(QueryBuilders.matchQuery("title", keywords))
                .withPageable(new PageRequest(0,10000));//只和知识标题进行匹配
        List<EsSearchBean> list = elasticsearchTemplate.queryForList(nativeSearchQueryBuilder.build(), EsSearchBean.class);
      //  long s2=System.currentTimeMillis();
    ...
    private List<KnowledgeContent> searchMonitorApiGMSL( String keywords, String oraginkeywords, List<KnowledgeContent> selectList){
        ...
        //long s3=System.currentTimeMillis();
        while(resultList.isEmpty() && keyLength > 0){
           // long s1=System.currentTimeMillis();
	        NativeSearchQueryBuilder nativeSearchQueryBuilder = new NativeSearchQueryBuilder();
	        nativeSearchQueryBuilder.withQuery(QueryBuilders.matchQuery("title", keywords)
                    .minimumShouldMatch(String.valueOf(keyLength)))
           ...
           // logger.debug("------------elasticsearchTemplate.queryForList1------------"+(s2-s1));
	            ...
	            for(KnowledgeContent kc : selectList){
	                for(EsSearchBean esb : list){
	                    if(kc.getId().equals(esb.getId()) && searchMonitorApiMK(oraginkeywords, kc)){
	                    	...
	                    	DecimalFormat df = new DecimalFormat("0.00%");
	                    	df.setRoundingMode(RoundingMode.HALF_UP);
	                    	double d = (double)keyLength/(double)wordLength(kc.getTitle());
	                    	m.put("match", df.format(d));
	                    	kc.setParams(m);//计算匹配率
	                        resultList.add(kc);
                            break;
	        ...
	        minuend2++;
	        keyLength = keywords.length() - minuend2;
      ...
      //根据字符串长度获取查询粒度
        ...
        while(resultList.isEmpty() && keyLength > 0){
            ...
            nativeSearchQueryBuilder.withQuery(QueryBuilders.matchQuery(name, keywords)
                    .minimumShouldMatch(String.valueOf(keyLength)))
                    .withPageable(new PageRequest(0,10000));
            resultList = elasticsearchTemplate.queryForList(nativeSearchQueryBuilder.build(), EsSearchBean.class);
